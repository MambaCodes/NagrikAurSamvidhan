...
let ques =["rights cannot be suspended ","abolishes Untouchability","Prohibition of traffic in human beings and forced labour","Equal opportunities for all sectors","India is not dependent on any country","Constitution draft committee","1960","All religions are equal in the eyes of the government"];
let ans =["Right to Life and Personal Liberty ","Article 17", "Right Against Exploitation", "social equality","Sovereign India", " K.M. Munshi","Berubari Case","Secular"];
...
const randomIndices = getRandomIndices(ques.length, 8);

const contentWithIndices = randomIndices.flatMap(index => [
  { index: index, content: ques[index] },
  { index: index, content: ans[index] }
]);
...
console.log('og' ,contentWithIndices);

// let shuffled = shuffle(contentWithIndices);
// console.log('new',shuffled);

// Create each card's HTMl
function createCard() {
  let cardList = shuffle(contentWithIndices);
  cardList.forEach(function(card) {
    $(".deck").append('<li><i class="card fa '+ card.index +'">'+card.content+'</i></li>');
  })
}
