...
const question = document.querySelector(".question .maindiv");
...
const getRandomWord = () => {
	if (words.length > 0) {
		//similarly getting the current question from object
...
const fillData = (jsonResponse, cat) => {
	if (cat === "pokemons") {
		...
		jsonResponse.forEach((obj) => {
			// words.push(obj.word);
			statement.push(obj.statement);
			words.push(obj.word);
		});
	}
	sessionStorage.setItem("currentData", JSON.stringify(words));
	sessionStorage.setItem("currentStats", JSON.stringify(statement));
};
const setupGame = () => {
	clearPrevWord(true);
	updateCurrent();
